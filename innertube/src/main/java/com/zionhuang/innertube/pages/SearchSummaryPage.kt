...
import com.zionhuang.innertube.models.removeIdentificationItem
    ...
    companion object {
        ...
        fun fromMusicResponsiveListItemRenderer(renderer: MusicResponsiveListItemRenderer): YTItem? {
            val secondaryLine = renderer.flexColumns.getOrNull(1)
                ?.musicResponsiveListItemFlexColumnRenderer?.text?.runs?.splitBySeparator()?.removeIdentificationItem()
                ?: return null
            return when {
                renderer.isSong -> {
                    SongItem(
                ...
                renderer.isAlbum -> {
                    AlbumItem(
                        ...
                        year = secondaryLine.getOrNull(1)?.firstOrNull()?.text?.toIntOrNull(),
                        thumbnail = renderer.thumbnail?.musicThumbnailRenderer?.getThumbnailUrl() ?: return null,
                        explicit = renderer.badges?.find {
                            it.musicInlineBadgeRenderer.icon.iconType == "MUSIC_EXPLICIT_BADGE"
                        } != null
